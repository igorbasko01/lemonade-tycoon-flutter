...
import 'package:lemonade_tycoon/buy_ingredients_screen.dart';
...
void main() {
  testWidgets('BuyIngredientsScreen displays balance and items', (WidgetTester tester) async {
    ...
    // Finds Buy buttons
    expect(find.text('Buy 1'), findsNWidgets(4)); // 4 ingredients in default supplier
  });

  testWidgets('Buy button updates balance and owned amount', (WidgetTester tester) async {
    await tester.pumpWidget(
      ChangeNotifierProvider(
        create: (_) => GameViewModel(),
        child: const MaterialApp(home: BuyIngredientsScreen()),
  ...
  testWidgets('Buy button is disabled when insufficient funds', (WidgetTester tester) async {
    ...
    final buyButton = tester.widget<ElevatedButton>(find.widgetWithText(ElevatedButton, 'Buy 1').first);
